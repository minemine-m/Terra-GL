...
import { ITileLoader, TileLoader } from "../tilesystem/loader";
import { ISource } from "../tilesystem/source";
...
export class TileMap extends Mesh<BufferGeometry, Material, TileMapEventMap> {
     ...
     * 地图创建工厂函数
       ...
       ```
     */
	public static create(params: TileMapParams) {
	 ...
	 * ```
	 */
	public constructor(params: TileMapParams) {
		super();
		this.up.set(0, 0, 1);
		const {
			loader = new TileLoader(),
