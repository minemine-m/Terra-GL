...
async function prepareAndCopy() {
	try {
		...
		const copyTasks = [
			{
				source: path.join(currentDir, "packages/core/dist"),
		...
		// 6. 将dist目录复制到example目录
		const exampleDistPath = path.join(currentDir, "example/dist");
		console.log(`准备将dist目录复制到example目录: ${exampleDistPath}`);

		// 确保example/dist目录存在
		await fs.mkdirp(exampleDistPath);

		// 清空example/dist目录
		await fs.emptyDir(exampleDistPath);

		// 将dist目录内容复制到example/dist
		await fs.copy(rootDistPath, exampleDistPath);
		console.log(`成功将dist目录复制到: ${exampleDistPath}`);
